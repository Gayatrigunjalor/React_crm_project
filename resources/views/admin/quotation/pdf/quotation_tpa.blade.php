        ...
        <table class="table" style="margin-top:10px;">
            <tr>
                <td style="text-align:center;">
                    <strong style="font-size: 14px;">TRI-PARTY AGREEMENT</strong><br>
                    <strong>THIS AGREEMENT IS MADE AND EXECUTED AT THIS {{ date("jS \of F Y", strtotime($quotation->pi_date)) }} AT PUNE,</strong><br>
            ...
            <tr>
                <td>
                    <strong>{{ $consigneeDetails->name ?? $buyerDetails->name }}</strong>, {{ $consigneeDetails->address ?? $buyerDetails->address }}, Email: {{ $consigneeDetails->email ?? $buyerDetails->email }}, {{ $consigneeDetails->mobile ?? $buyerDetails->contact_no }},    here in after    referred    to    as    the “<strong>Consignee</strong>”; (Which expression shall unless repugnant to the context, mean and include his heirs, executors, administrators and assigns) Party of the Second Part.
                </td>
            </tr>
            <tr>
                <td style="text-align:center;">
            ...
            <tr>
                <td>
                    <strong>AND WHEREAS</strong>, Buyer desires to get the abovementioned goods delivered on Consignee’s delivery address and this desire is free from any undue influence by any party.<br>
                    <strong>AND WHEREAS</strong>, Buyer understands that Seller is not responsible for Delivery of the Goods at Consignee’s address.<br>
            ...
            <tr>
                <td>
                    <strong>Sale of Goods:</strong> That, Seller agrees to sell, transport and deliver to Buyer, and Buyer agrees to purchase the following items at <strong>{{ $symbol->symbol }} {{ number_format($quotation->grand_total, 2) }}</strong> the Price mentioned under Proforma Invoice No. <strong>{{ $quotation->pi_number }}</strong> on dated <strong>{{ date("d/m/Y", strtotime($quotation->pi_date)) }}</strong> (hereinafter referred to as the “Said Goods”). Incoterm is {{ $incoTerm->inco_term }}.<br>
                </td>
            </tr>
            <tr>
                <td>
            ...
            <table class="table" style="font-size:11px;width:100%;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-size:11px;">{{ $quotationProduct->hsn }}</td>
                        <td style="font-size:11px;">{{ $quotationProduct->quantity }}</td>
                        <td style="font-size:11px;">{{ number_format($quotationProduct->rate, 2) }}</td>
                        <td style="font-size:11px;">{{ number_format($quotationProduct->amount, 2) }}</td>
                    </tr>
                    @endforeach
                    <tr>
                        <td style="font-size:11px;" colspan="5"><strong>TOTAL PRODUCT COST / EXW VALUE</strong></td>
                        <td style="font-size:11px;"><strong>{{ number_format($quotation->total, 2) }}</strong></td>
                    </tr>
                    <tr>
                        <td style="font-size:11px;" colspan="5"><strong>FREIGHT</strong></td>
                        <td style="font-size:11px;"><strong>{{ number_format($quotation->shipping_cost, 2) }}</strong></td>
                    </tr>
                    {{-- <tr>
                        <td style="font-size:11px;" colspan="5"><strong>INSURANCE</strong></td>
                        <td style="font-size:11px;"><strong>{{ number_format($quotation->insurance, 2) }}</strong></td>
                    </tr> --}}
                    <tr>
                        <td style="font-size:11px;" colspan="5"><strong>GRAND TOTAL</strong></td>
                        <td style="font-size:11px;"><strong>{{ $symbol->symbol }} {{ number_format($quotation->grand_total, 2) }}</strong></td>
                    </tr>
                </tbody>
            </table>
        </section>
        ...
        <table class="table" style="margin-top:5px;">
            ...
            <tr>
                <td>
	                ...
	                All right reserved by seller, seller can cancel an order without any notice & Buyer liable to indemnify the seller for all the "Losses" mentioned above but not limiting to that only.
                </td>
            </tr>
            <tr>
                <td>
            ...
            <tr>
                <td>
                    <strong>Governing Law and Arbitration:</strong> That, This Agreement shall be governed by the laws of India. The Courts in Pune, Maharashtra, India shall have exclusive jurisdiction over the subject matter of this Agreement. In the event of any dispute or differences arising out of or in connection with this agreement, the parties hereto, agree to resolve their dispute by a sole arbitrator appointed and selected by parties. Arbitration shall be conducted in accordance with the provisions of the Arbitration and Conciliation Act, 1996 or any other subsequent modifications or enactments thereof.
                </td>
            </tr>
            <tr>
                <td>
                    &nbsp;&nbsp;&nbsp;&nbsp;(a)	That, the arbitration proceedings shall be conducted in English. The place of Arbitration shall be in Pune, Maharashtra, India. The award passed in the arbitration proceedings shall be final and binding on all the parties.
                </td>
            </tr>
            <tr>
                <td>
            ...
            @if ($consigneeDetails->name != null)
            <tr>
                <td>
                    Sd/-<br><br><br><br><br><br><br><br><br>
                    Consignee, <strong>{{ $consigneeDetails->name }}</strong><br><br>
                </td>
            </tr>
            @endif
            <tr>
