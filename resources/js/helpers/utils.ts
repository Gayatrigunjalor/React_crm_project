...
export const formatDateToDayMonthYear = (dateInput: string | Date): string => {
    const date = dateInput instanceof Date ? dateInput : new Date(dateInput);

    if (isNaN(date.getTime())) {
        throw new RangeError('Invalid time value');
        // Or return a fallback string like ''
        // return '';
    }
    ...
    const formatted = formatter.format(date); // e.g. "09 May 2025"
    return formatted.replace(/ /g, '-'); // "09-May-2025"
};
