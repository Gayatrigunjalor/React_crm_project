...
const getDefaultOptions = (getThemeColor: (name: string) => string, activeCount: number, inactiveCount: number) => ({
  color: [
  ...
  series: [
    {
      ...
      data: [
        { value: activeCount, name: 'Active Employees' },
        { value: inactiveCount, name: 'Inactive Employees' },
      ]
    }
  ],
...
const EcomTopCouponsChart = ({ activeCount, inactiveCount }) => {
  const { getThemeColor } = useAppContext();

  return (
    <ReactEChartsCore
      echarts={echarts}
      option={getDefaultOptions(getThemeColor, activeCount, inactiveCount)}
