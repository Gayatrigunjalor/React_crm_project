...
import CustomerCountTopChart from '../charts/e-charts/CustomerCountTopChart';
import { Card } from 'react-bootstrap';

const  CustomerCountCard = () => {
 
  const [count, setTotalCustomerCount] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTotalCustomerCount(results[0]);
    ...
    <Card className="h-100">
      <Card.Body>
        <div className="d-flex justify-content-between">
          <div>
            <h5 className="mb-2">Total Customers</h5>
          </div>
        </div>
        <div className="pb-4 pt-3">
        <CustomerCountTopChart totalCount={count}  />
        </div>
        <div>
