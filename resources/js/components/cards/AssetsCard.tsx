...
import AssetTopChart from '../charts/e-charts/AssetTopChart';
import { Card } from 'react-bootstrap';

const AssetsCard = () => {
 
  const [totalCount, setTotalAssetCount] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { totalAssetCount,totalAssignedCount, totalUnassignedCount } = results[0];
      
        // Update the state with the fetched counts
        setTotalAssetCount(totalAssetCount);
    ...
    <Card className="h-100">
      <Card.Body>
        <div className="d-flex justify-content-between">
          <div>
            <h5 className="mb-2">Total Assets</h5>
          </div>
        </div>
        <div className="pb-4 pt-3">
        <AssetTopChart totalCount={totalCount} assignedCount={assignedCount} unassignedCount={unassignedCount} />
        </div>
        <div>
