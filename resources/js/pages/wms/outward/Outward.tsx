...
import OutwardTable, { outwardTableColumns } from './OutwardTable';
...
export interface OutwardDataType {
    id: number;
...
const Outward = () => {
    const [outwardData, setOutwardData] = useState<OutwardDataType[]>([]);
    ...
    const handleCreateOutward = () => {
        navigate(`/warehouse-outward/create`);
    };

    const handleOutwardEdit = (outwardId: number) => {
        navigate(`/warehouse-outward/create/${outwardId}`);
    };

    const handlePDFclicked = async (outwardId: number, path: string) => {
        try {
            // Fetch the file from the server using the ID
            const response = await axiosInstance.get(`/${path}/${outwardId}`, {
                method: 'GET',
                responseType: 'blob',
            });
    ...
    const handleOutwardDelete = async ( outward_id: number) => {
        swal({
            title: "Are you sure?",
        ...
        .then((willDelete) => {
            if (willDelete) {
                axiosInstance.delete(`/warehouse-outward/${outward_id}`)
                .then(response => {
    ...
    const table = useAdvanceTable({
        data: outwardData,
        columns: outwardTableColumns(handleOutwardEdit, handlePDFclicked, handleOutwardDelete),
        pageSize: 10,
        pagination: true,
        sortable: true,
        selection: false,
    ...
    useEffect(() => {
        const fetchOutwardData = async () => {
            try {
                const response = await axiosInstance.get('/warehouse-outward'); // Replace with your API URL
                if (response.status !== 200) {
                    throw new Error('Failed to fetch data');
                }
                const data: OutwardDataType[] = await response.data;
                setOutwardData(data);
            } catch (err: any) {
