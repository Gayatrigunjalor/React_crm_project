...
import { OutwardDataType } from './Outward';
import { useAuth } from '../../../AuthContext';

export const outwardTableColumns = (handleOutwardEdit: (outwardId: number) => void, handlePDFclicked: (outwardId: number, path: string) => void, handleOutwardDelete: (outwardId: number) => void): ColumnDef<OutwardDataType>[] => [
    {
        id: 'outward_sys_id',
        header: () => {
            return (
                <div className="d-inline-flex flex-center">
                    <span>Outward ID</span>
                </div>
            );
        },
        cell: ({ row: { original } }) => {
            const { outward_sys_id, outward_date } = original;
            return (
                <>
                <span>{outward_sys_id}</span> <br />
                <span>{outward_date}</span>
                </>
            );
        },
        meta: {
        ...
        cell: ({ row: { original } }) => {
            ...
            return (
                ...
                {userPermission.warehouse_edit == 1 && (
                    <Button className='text-success' variant='link' title='Edit Outward' onClick={() => handleOutwardEdit(id)} startIcon={<FontAwesomeIcon icon={faEdit} />}></Button>
                )}
                {userPermission.warehouse_delete == 1 && (
                    <Button className='text-danger' variant='link' title='Delete Outward' onClick={() => handleOutwardDelete(id)} startIcon={<FontAwesomeIcon icon={faTrash} />}></Button>
                )}
                {userPermission.warehouse_list == 1 && (
                    <Button className='text-info' variant='link' title='Print Sticker' onClick={() => handlePDFclicked(id, 'create-outward-sticker')} startIcon={<FontAwesomeIcon icon={faPrint} />}></Button>
                )}
                </>
