...
import { SegmentDataType } from './Segment';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/segment/${userId}`)
            .then(response => {
...
export const segmentTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<SegmentDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const SegmentTable: React.FC<SegmentTableProps> = ({ handleShow, handleSuccess }) => {
