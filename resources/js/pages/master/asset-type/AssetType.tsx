...
import AssetTypeTable, { assetTypeTableColumns } from './AssetTypeTable';
...
const AssetType = () => {
    const [assetTypeTableData, setAssetTypeTableData] = useState<AssetTypeDataType[]>([]);
    ...
    const table = useAdvanceTable({
        ...
        columns: assetTypeTableColumns(handleShow, handleSuccess),
        pageSize: 10,
        pagination: true,
        sortable: true,
        selection: false,
    ...
    useEffect(() => {
        const fetchAssetType = async () => {
            try {
                ...
                const data: AssetTypeDataType[] = await response.data;
                setAssetTypeTableData(data);
            } catch (err: any) {
            ...
            <Card>
                <Card.Body>
                    <AdvanceTableProvider {...table}>
                        ...
                        <AssetTypeTable handleShow={handleShow} handleSuccess={handleSuccess} />
                    </AdvanceTableProvider>
                </Card.Body>
            </Card>
            {showModal && (
                <AssetTypeModal userId={selectedUserId} onHide={handleClose} onSuccess={handleSuccess} />
            )}
        </>
    )
};
