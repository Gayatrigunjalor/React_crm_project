...
import { AssetTypeDataType } from './AssetType';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/asset-type/${userId}`)
            .then(response => {
...
export const assetTypeTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<AssetTypeDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const AssetTypeTable: React.FC<AssetTypeTableProps> = ({ handleShow, handleSuccess }) => {
