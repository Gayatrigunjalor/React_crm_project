...
import { HsnCodeDataType } from './HsnCode';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/hsn-code/${userId}`)
            .then(response => {
...
export const hsnCodeTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<HsnCodeDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const HsnCodeTable: React.FC<HsnCodeTableProps> = ({ handleShow, handleSuccess }) => {
