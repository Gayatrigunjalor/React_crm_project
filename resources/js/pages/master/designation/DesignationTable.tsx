...
import { DesignationDataType } from './Designation';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/designation/${userId}`)
            .then(response => {
...
export const designationTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<DesignationDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
