...
import { CustomerBaseDataType } from './CustomerBase';
...
interface CustomerBaseTableProps {
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete('/deleteCustomerBase', {
                params: {
                    id: userId
                }
            })
...
export const customerBaseTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<CustomerBaseDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
        ...
        cell: ({ row: { original } }) => {
            ...
            return (
                <div className="d-flex align-items-center gap-2">
                    <Button variant='outline-secondary' onClick={() => handleShow(id)} startIcon={<FontAwesomeIcon icon={faEdit} />}></Button>
...
const CustomerBaseTable: React.FC<CustomerBaseTableProps> = ({ handleShow, handleSuccess }) => {
...
export default CustomerBaseTable;
