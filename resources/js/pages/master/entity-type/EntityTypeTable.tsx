...
import { EntityTypeDataType } from './EntityType';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/entity-type/${userId}`)
            .then(response => {
...
export const entityTypeTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<EntityTypeDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const EntityTypeTable: React.FC<EntityTypeTableProps> = ({ handleShow, handleSuccess }) => {
