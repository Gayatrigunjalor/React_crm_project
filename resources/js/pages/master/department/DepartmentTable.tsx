...
import { DepartmentDataType } from './Department';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/department/${userId}`)
            .then(response => {
...
export const departmentTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<DepartmentDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
