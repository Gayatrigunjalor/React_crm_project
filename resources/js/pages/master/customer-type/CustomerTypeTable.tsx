...
import { CustomerTypeDataType } from './CustomerType';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/customer-type/${userId}`)
            .then(response => {
...
export const customerTypeTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<CustomerTypeDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const CustomerTypeTable: React.FC<CustomerTypeTableProps> = ({ handleShow, handleSuccess }) => {
