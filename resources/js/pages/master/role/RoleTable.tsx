...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/role/${userId}`)
            .then(response => {
...
export const roleTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<RoleDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const RoleTable: React.FC<RoleTableProps> = ({ handleShow, handleSuccess }) => {
