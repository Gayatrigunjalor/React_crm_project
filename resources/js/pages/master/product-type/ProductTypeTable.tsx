...
import { ProductTypeDataType } from './ProductType';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/product-type/${userId}`)
            .then(response => {
...
export const productTypeTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<ProductTypeDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const ProductTypeTable: React.FC<ProductTypeTableProps> = ({ handleShow, handleSuccess }) => {
