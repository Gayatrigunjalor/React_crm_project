...
import { LocationDetailDataType } from './LocationDetail';
import { faEdit, faTrash } from '@fortawesome/free-solid-svg-icons';
import swal from 'sweetalert';
import axiosInstance from '../../../axios';

interface LocationDetailTableProps {
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/location-detail/${userId}`)
            .then(response => {
...
export const locationDetailTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<LocationDetailDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
    ...
    },
    {
        ...
        cell: ({ row: { original } }) => {
            const { rack_number } = original;
            return (
                <span>{rack_number}</span>
            );
        },
        meta: {
            headerProps: {
...
const LocationDetailTable: React.FC<LocationDetailTableProps> = () => {
