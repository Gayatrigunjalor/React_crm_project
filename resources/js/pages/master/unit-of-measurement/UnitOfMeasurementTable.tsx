...
import { UnitOfMeasurementDataType } from './UnitOfMeasurement';
...
const handleDelete = (userId: number, handleSuccess: () => void) => {
    ...
    .then((willDelete) => {
        if (willDelete) {
            axiosInstance.delete(`/unit-of-measurements/${userId}`)
            .then(response => {
...
export const unitOfMeasurementTableColumns = ( handleShow: (userId?: number) => void, handleSuccess: () => void): ColumnDef<UnitOfMeasurementDataType>[] => [
    {
        accessorKey: 'index',
        header: () => {
            return (
...
const UnitOfMeasurementTable: React.FC<UnitOfMeasurementTableProps> = ({ handleShow, handleSuccess }) => {
