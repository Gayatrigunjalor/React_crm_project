...
const KanbanContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log("Fetching data in Collegue Task ...");
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    // Dispatch action to move items in the state
    kanbanDispatch({
      type: 'MOVE_ITEMS',
    ...
    try {
      ...
      // Find the source stage to determine if it's a task or subtask
      const sourceStage = data.assignStages.find((stage: any) => stage.id.toString() === sourceId);
      const isSubtask = sourceStage?.assignSubtasks?.some((subtask: any) => subtask.id.toString() === taskId);

      // Choose the appropriate API endpoint and payload based on whether it's a task or subtask
      const endpoint = isSubtask ? '/updateStageNextSubtask' : '/updateKanbanStatus';
  ...
  const fetchTasks = async () => {
    ...
    if (selectedFilter === "due_date" || selectedFilter === "date_created" || selectedFilter === "date_updated" || selectedFilter === "start_date") {
    ...
    try {
      ...
      setData(response.data);
      setOpenFilterModal(false); // Close the filter modal after applying filters
      console.log('DATA IS', response.data);
    } catch (err) {
      setError('Error fetching data');
      console.error(err);
              ...
              {(provided) => (
                <div ref={provided.innerRef} {...provided.droppableProps}>
                  <KanbanList
                    list={list}
                    key={list.id}
                    dataObj={data.assignStages[s]}
                    empInf={employees}
                    refreshTasks={fetchUpdatedData}
                    showAddTaskButton={false}
                    isTask={false}
                    searchQuery={searchQuery} // Pass search query to KanbanList
                  />
