...
const PrimaryDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.get('/kpiGraphsLevelZero');
    ...
    // Bar chart options with datalabels positioned above bars to ensure visibility
    ...
    // Chart data for bar charts - unchanged from user code with safety fallback to 0
    ...
    // Set doughnut center to always show 100%
    const calculateTotalPercentage = () => {
        return 100;
    };

    // Prepare performance metrics data for progress bars
    const performanceMetrics = dashboardData
        ? [
              {
                  label: 'Total KPIs Assigned',
                  value: dashboardData.performanceMatrixCounts.kpisTotalCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalCount,
              },
              {
                  label: 'KPI: Target in Number',
                  value: dashboardData.performanceMatrixCounts.kpisTotalNumberCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalNumberCount,
              },
              {
                  label: 'KPI: Target Done/Not-Done',
                  value: dashboardData.performanceMatrixCounts.kpisTotalDndCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalDndCount,
              },
              {
                  label: 'KPI: Target in Currency',
                  value: dashboardData.performanceMatrixCounts.kpisTotalCurrencyCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalCurrencyCount,
              },
              {
                  label: 'KPI: Target in Number - Completed',
                  value: dashboardData.performanceMatrixCounts.kpisCompletedNumberCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalNumberCount,
              },
              {
                  label: 'KPI: Target Done/Not Done - Completed',
                  value: dashboardData.performanceMatrixCounts.kpisCompletedDndCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalDndCount,
              },
              {
                  label: 'KPI: Target in Currency - Completed',
                  value: dashboardData.performanceMatrixCounts.kpisCompletedCurrencyCount,
                  max: dashboardData.performanceMatrixCounts.kpisTotalCurrencyCount,
              },
          ]
        : [];
    ...
    return (
        <>
            {/* Add global styles to make cards same height in each row */}
            <style>{`
                .row-equal-height {
                    display: flex;
                    flex-wrap: wrap;
            ...
            <Container fluid className="p-4">
                <Row className="mb-4 row-equal-height">
                    <Col md={6} className="d-flex">
                        <h3 className="mb-5 w-100">KPI Status</h3>
                        <Card className="p-4 shadow rounded-3 same-height-card">
                            <Bar options={barChartOptions} data={kpiStatusData} height={200} plugins={[ChartDataLabels]} />
                        </Card>
                    </Col>
                    <Col md={6} className="d-flex">
                        <h3 className="mb-5 w-100">KPI Task Priority</h3>
                        <Card className="p-4 shadow rounded-3 same-height-card">
                ...
                <Row className="mb-4 row-equal-height">
                    <Col md={6} className="d-flex">
                        <h3 className="mb-3 w-100">Target-wise Progress Overview</h3>
                        <Card className="p-3 shadow rounded-4 same-height-card">
                            <Bar options={barChartOptions} data={targetProgressData} height={200} plugins={[ChartDataLabels]} />
                        </Card>
                    </Col>
                    <Col md={6} className="d-flex">
                        <h3 className="mb-3 w-100">Target Distribution</h3>
                        <Card className="p-3 shadow rounded-4 d-flex flex-row align-items-center same-height-card" style={{ minHeight: 340 }}>
                            <div className="position-relative" style={{ width: 240, height: 240 }}>
                                ...
                                <div
                                    ...
                                    style={{ fontSize: 36, color: '#222', userSelect: 'none' }}
                                >
                ...
                <Row className="row-equal-height">
                    <Col className="d-flex flex-column">
                        ...
                        <Card className="p-4 shadow rounded-4 same-height-card">
                            {performanceMetrics.map((item, idx) => (
                                <div key={item.label} className={idx === 0 ? '' : 'mt-3'}>
                                    <div className="d-flex justify-content-between align-items-center mb-1">
                                        <span>{item.label}</span>
                                        <span className="fw-bold" style={{ minWidth: 32, textAlign: 'right' }}>
                                            {item.value}
                                        </span>
                                    </div>
                                    <ProgressBar
