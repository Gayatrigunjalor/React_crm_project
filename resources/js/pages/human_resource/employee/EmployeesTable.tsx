...
export const employeesTableColumns = (handleShow: (userId?: number) => void, handleTaskCheckbox: (empId: number, empName: string, currentState: boolean) => void, handlePermissionsModal: (userId: number) => void, handleDashboardPermissions: (userId: number, name: string) => void, handleChangePassword: (userId: number) => void, handleTwoFactorAuthentication: (userId: number) => void, handleDeleteEmployeeTask: (userId: number, name: string) => void, handleAssignAsset: (empId: number) => void, handleAssignCredentials: (empId: number) => void): ColumnDef<EmployeesDataType>[] => [
    {
        accessorKey: 'emp_id',
        header: () => {
            return (
    ...
    },
    {
        ...
        accessorFn: ({ department }) => department ? department.name : '',
        // cell: ({ row: { original } }) => {
            ...
            return (
                <div className="d-flex align-items-center gap-1">
                    ...
                    {empData.emp_user.user_role == 0 && (
                        <>
                            <Button variant='link' title='Edit permissions' onClick={() => { handlePermissionsModal(user_id) }} startIcon={<FontAwesomeIcon icon={faParking} />} ></Button>
