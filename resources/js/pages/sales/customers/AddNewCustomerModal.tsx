...
import AddCustomerConsigneesModal from '../../sales/customers/AddCustomerConsigneesModal';
...
const AddNewCustomerModal: React.FC<CustomerModalProps> = ({ onHide, onSuccess, leadId, customer_id }) => {
    ...
    const [openConsigneeAfterClose, setOpenConsigneeAfterClose] = useState(false);
    const [isModalVisible, setIsModalVisible] = useState(true);

    // const [consigneeSameAsCustomer, setConsigneeSameAsCustomer] = useState(false);
    ...
    //     setConsigneeSameAsCustomer(true);

    //     if (consigneeSameAsCustomer) {
    //         setConsigneeData({
    //             name: consigneeData.name || '',
    //             cust_id: consigneeData.cust_id || '',
    //             city: consigneeData.city || '',
    //             pin_code: consigneeData.pin_code || '',
    //             email: consigneeData.email || '',
    //             contact_no: consigneeData.contact_no || '',
    //             contact_person: consigneeData.contact_person || '',
    //             address: consigneeData.address || '',
    //             designation: consigneeData.designation || '',
    //         });
    //     } else {
    //         setConsigneeData({
    //             name: '',
    ...
    const handleClose = () => setShowConsigneeModal(false);
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true);
        setValidated(true);
        const apiCall = axiosInstance.post('/customers', {
            ...custData,
        ...
        apiCall
            .then((response) => {
                ...
                swal({
                    ...
                    if (value === "addConsignee") {
                        setCustomerId(id);
                        setOpenConsigneeAfterClose(true);
                    }
...
`;
    ...
    useEffect(() => {
        if (!isModalVisible && openConsigneeAfterClose) {
            setShowConsigneeModal(true);
            setOpenConsigneeAfterClose(false);
        }
    }, [isModalVisible, openConsigneeAfterClose]);
        ...
        <>
         <style>{placeholderStyle}</style>
            <Modal show={isModalVisible}
                onHide={() => {
                    setIsModalVisible(false);
                    onHide(); // This is your prop function to inform the parent
                }}
                onExited={() => {
                    if (openConsigneeAfterClose) {
                        setShowConsigneeModal(true);
                        setOpenConsigneeAfterClose(false);
                    }
                }}
            ...
            {showConsigneeModal && (
                <AddCustomerConsigneesModal id={customerId} onHide={handleClose} leadId={leadId} cusId={customer_id} />
            )}
        </>
    );
};
