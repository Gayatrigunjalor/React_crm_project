...
import CalendarTop from './ReminderCalTop';
import CalendarHeader from './ReminderCalHeader';
import CalendarEventModal from '../../../../components/modals/CalendarEventModal';
import { HANDLE_SELECT, SET_CALENDAR_STATE } from '../../../../reducers/CalendarReducer';
import FullCalendarReminder from '../../../../components/base/FullCalendarReminder';
...
import { startOfMonth, startOfWeek, differenceInCalendarWeeks, getDay } from 'date-fns';
import { getDate } from 'date-fns';

const ReminderCalDashboard = () => {
  return (
    <CalendarProvider>
      <Calendar />
    </CalendarProvider>
...
const Calendar = () => {
  ...
  const getWeekOfMonth = (date: Date) => {
    ...
    // Adjust so that weeks are always counted from 1
    return Math.ceil((date.getDate() + firstDayWeekday - 1) / 7) || 1;
  };
  ...
  const fetchCalendarData = async (viewType: CalendarView) => {
    ...
    if (viewType === 'dayGridMonth') {
      endpoint = '/reminder_monthly';
    } else if (viewType === 'timeGridWeek') {
      endpoint = '/reminder_weekly';
      params.week = week;
    }
  ...
  return (
    <div>
      ...
      <div className="mt-6 mb-9">
        <FullCalendarReminder calendarData={calendarData} height={800} select={info => calendarDispatch({ type: HANDLE_SELECT, payload: info })} eventClick={handleEventClick} />
      </div>
      <CalendarEventModal />
...
export default ReminderCalDashboard;
