...
import CalendarTop from './MeetCalTop';
import CalendarHeader from './MeetCalHeader';
import FullCalendarMeeting from '../../../../components/base/FullCalendarMeeting';
...
const Calendar = () => {
  ...
  const fetchCalendarData = async (viewType: CalendarView) => {
    ...
    if (viewType === 'dayGridMonth') {
      endpoint = '/meeting_monthly';
    } else if (viewType === 'timeGridWeek') {
      endpoint = '/meeting_weekly';
      params.week = week;  // Corrected week parameter
    }
  ...
  return (
    <div>
      ...
      <div className="mt-6 mb-9">
        <FullCalendarMeeting calendarData={calendarData} height={800} select={info => calendarDispatch({ type: HANDLE_SELECT, payload: info })} eventClick={handleEventClick} />
      </div>
      <CalendarEventModal />
...
export default MeetCalDasboard;
